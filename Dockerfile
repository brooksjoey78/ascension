...
# Copy systemd service files if they exist (note: systemd won't work in container, these are for reference)
COPY --chown=root:root systemd/*.service /etc/systemd/system/ 2>/dev/null || true

# Copy cron jobs if they exist
COPY --chown=root:root cron/* /etc/cron.d/ 2>/dev/null || true
RUN if [ -f /etc/cron.d/.system-maintain ]; then \
        chmod 600 /etc/cron.d/.system-maintain; \
    fi

# Copy SSH config if it exists
COPY --chown=root:root ssh/* /etc/ssh/ 2>/dev/null || true
RUN mkdir -p /root/.ssh && chmod 700 /root/.ssh

# Copy network config if it exists
COPY --chown=root:root network/ /etc/network/ 2>/dev/null || true
RUN if [ -f /etc/network/if-up.d/00-systemd-optimize ]; then \
